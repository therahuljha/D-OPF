...
function [xnl,VAlin, VBlin, VClin, xlin, fvallin,TableAlin,TableBlin,TableClin, VolttableAlin,VolttableBlin,VolttableClin,error,fvalue] = firstSOCPforintialpointusingCVX(t)
...
xintial = zeros(1235,1);   
%% intial points for optimization
 xintial(1:182,1)= xlin(1:182,1);
 xintial(366:523,1)= xlin(275:432,1);
 xintial(683:858,1)= xlin(513:688,1);
...
for k = 1:size(powerdata,1)
    if powerdata(k,11) ~= 0 && powerdata(k,12) ~= 0 && powerdata(k,13) ~= 0
        ParentA = find(k == TA(:,2));
         Aeq(TableAlin(ParentA ,4),totalpfvar+p) = 1;
         p = p+1;
         ParentB = find(k == TB(:,2));
         Aeq(TableBlin(ParentB ,4),totalpfvar+p) = 1;
         p = p+1;
         ParentC = find(k == TC(:,2));
         Aeq(TableClin(ParentC ,4),totalpfvar+p) = 1;
         p = p+1;
...
for i =2:nb
   ...
   A1(indexA+ParentA,TableA(ParentA,5)) = -1;
    b1(indexA+ParentA)= -xintial(TableA(ParentA,5)) + deli;
%     b1(indexA+ParentA)= 0;
end
...
for i =2:nb
   ...
   b1(ParentA+indexA)= deli + xintial(TableA(ParentA,5));
end
...
for i =2:nb
   ...
   A1(indexA+ParentB,TableB(ParentB,5)) = -1;
    b1(indexA+ParentB)= -xintial(TableB(ParentB,5)) + deli;
...
for i =2:nb
   ...
   b1(ParentB+indexA)= deli + xintial(TableB(ParentB,5));
end
...
A1(indexA+p,totalpfvar+i) = 1;
A1(indexA+p+1,totalpfvar+i) = -1;
b1(indexA+p) = Q_limit;
